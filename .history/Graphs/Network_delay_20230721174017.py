...
def NetworkDelay(times, N, K):
    ...
    #*Create distance dictionary and fill starting node
    distance = {node: float('infinity') for node in range (1,N+1)}
    distance[K]= 0
    ...
    while priority_queue:
        ...
        #*Process Node only if current_dist < saved dist
        if current_distance <= distance[current_node]:
            for neighbor, weight in graph[current_node]:
                distance_to_neighbor = current_distance + weight
                
                #*if calculated dist < recorded dist, update shortest dist, add neighbour to priority queue
                if distance_to_neighbor < distance[neighbor]:
                    distance[neighbor] = distance_to_neighbor
                    heapq.heappush (priority_queue, (distance_to_neighbor, neighbor))
